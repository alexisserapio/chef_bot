import 'package:flutter/widgets.dart';

class AppColors {
  static const Color backgroundColor = Color.fromRGBO(255, 248, 226, 1);
  //Button Colors
  static const Color saveColor = Color.fromRGBO(255, 0, 0, 1);
  static const Color sendColor = Color.fromRGBO(139, 195, 73, 1);
  //Message Colors
  static const Color userMessages = Color.fromRGBO(232, 237, 194, 1);
  static const Color botMessages = Color.fromRGBO(255, 255, 255, 1);
  static const Color imgBackground = Color.fromRGBO(235, 206, 174, 1);
  //Others
  static const Color white = Color.fromRGBO(255, 255, 255, 1);
  static const Color black = Color.fromRGBO(0, 0, 0, 1);
  static const Color orange = Color.fromRGBO(255, 168, 8, 1);
  static const Color red = Color.fromRGBO(255, 0, 0, 1);
  static const Color grey = Color.fromRGBO(158, 158, 158, 1);
  static const Color green = Color.fromRGBO(46, 172, 46, 1);
  static const Color ambar = Color.fromRGBO(255, 204, 62, 1);
}
